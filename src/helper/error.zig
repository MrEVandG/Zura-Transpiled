...
//TODO: Add in a way to keep track of the current file we are parsing and lexing
//      so that we can print out the file name in the error message
...
pub fn lError(line: usize, pos: usize, _msg: []const u8) void {
    comptime var cham = Chameleon.init(.Auto);
    print("[{}::{}] ", .{ line, pos });
...
pub fn pError(line: usize, pos: usize, _msg: []const u8) void {
    comptime var cham = Chameleon.init(.Auto);
    print("[{}::{}] ", .{ line, pos });
    print(cham.red().fmt("error:"), .{});
    print(" {s}\n", .{_msg});
